...
func main() {
  ...
  if len(resp.Values) > 0 {
    t := time.Now()
    today := fmt.Sprintf("%d-%s", t.Day(), t.Format("Jan-2006"))
    fmt.Printf("The date today is: %s\n", today)
    for _, row := range resp.Values {
      start := row[0]
      end := row[1]
      username := row[2]
      if start == today {
        msg := fmt.Sprintf("@%s is on call starting today\n", username)
        fmt.Printf(msg)
        // slackNotify(msg)
      }
      if end == today {
        msg := fmt.Sprintf("@%s on call assignment ends tonight\n", username)
        fmt.Printf(msg)
        // slackNotify(msg)
      }
      // fmt.Printf("%s, %s, %s\n", row[0], row[1], row[2])
